...
### 项目概述
**摩斯电码手电筒 (Morse Light)** 是一款基于 React、Vite 和 Capacitor 构建的跨平台移动应用程序。它允许用户使用设备的闪光灯或屏幕发送摩斯电码信息。该应用专为紧急情况和教育目的设计，拥有简洁现代的用户界面，支持16种语言，并包含专门的教程部分。
...
- **`src/`**：源代码根目录。
  ...
  - **`i18n/`**：国际化配置和翻译文件。
  - **`contexts/`**：React Context 提供者（语言、设置）。
...
1. 转到 **首页 (Home)** 标签。
2. 在文本框中输入 "HELLO"。
3. 点击 **开始传输 (START TRANSMISSION)**。
4. 手电筒（或屏幕）将闪烁 "HELLO" 的摩斯电码。
...
### 配置选项
- **语言**：在 **设置** 中使用下拉菜单从16种语言中选择。应用首次启动时会自动检测您的设备语言。
- **传输模式**：在 **设置** 中切换 "手电筒" 和 "屏幕闪烁"。
...
**问：我可以添加更多语言吗？**
答：可以！在 `src/i18n/translations.ts` 中的 `LANGUAGE_OPTIONS` 和 `translations` 中添加新语言即可。

**问：倒计时是如何工作的？**
答：使用屏幕闪烁模式时，传输开始前会出现3-2-1倒计时，让您有时间做好准备。
